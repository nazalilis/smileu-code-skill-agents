...
- **Release automation.** Pick a release type (patch, minor, major, prepatch, preminor,
  premajor, prerelease) or an exact version from the GitHub Actions UI. Releases also
  run when a push to `main` changes the version in `package.json`, or when a `vX.Y.Z`
  tag is pushed. Release notes come from this changelog. Re-runs update the existing
  GitHub Release and skip versions already on the registry; prereleases are marked as
  such and published with the `next` dist-tag.
...
- **Dependabot** for npm dependencies and GitHub Actions versions.
