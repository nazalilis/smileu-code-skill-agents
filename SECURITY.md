...
6. **Network access:** the CLI only goes online for `--latest` (git clone from GitHub), `update --check` (npm registry, then the GitHub Releases API) and `audit` (`npm audit`). Nothing else is sent anywhere.
...
- Unquoted `KEY=value` secrets in `.env` files (not in `.env.example`, `.sample` or `.template`). Keys that describe a secret rather than hold one, such as `TOKEN_URL` or `SECRET_FILE`, are not reported.
- `eval()`, `new Function()`, child process calls with a `shell` option, and `exec`/`execSync` commands built from template strings or concatenation. Comment lines are ignored.
...
- Workflow actions are pinned to full commit SHAs, and the release workflow never commits to the repository.
